...
public class Project implements Comparable<Project> {
    ...
    /**
     * Maps the number of working days per month
     * between start and end date of a single project.
     *
     * @return the number of workingsdays for each month
     */
    public Map<Month, Integer> getNumOfWorkingDaysInMonth() {
        Map<Month, Integer> nWorkingDaysPerMonth = new TreeMap<>();
        LocalDate firstDay = this.startDate;
        LocalDate lastDay = this.endDate;

        // The amount of working days in the starting month and last month
        int firstMonthDays = utils.Calendar.getNumWorkingDays(firstDay, YearMonth.from(firstDay).atEndOfMonth());
        int lastMonthDays = utils.Calendar.getNumWorkingDays(YearMonth.from(lastDay).atDay(1), lastDay);

        nWorkingDaysPerMonth.put(firstDay.getMonth(), firstMonthDays);
        nWorkingDaysPerMonth.put(lastDay.getMonth(), lastMonthDays);

        YearMonth l1 = YearMonth.from(firstDay).plusMonths(1);
        YearMonth l2 = YearMonth.from(lastDay);

        // Loop trough the months between the start month and the ending month
        while (l1.isBefore(l2)) {
            nWorkingDaysPerMonth.put(l1.getMonth(), utils.Calendar.getNumWorkingDays(l1.atDay(1), l1.atEndOfMonth()));
            l1 = l1.plusMonths(1);
        }

        return nWorkingDaysPerMonth;
    }
    ...
    /**
     * Maps the budget per month for a single project.
     *
     * @return This project's monthly manpower
     */
    public Map<Month, Integer> getManPowerBudgetPerMonth() {
        int totalSalaryPerDay = committedHoursPerDay.entrySet().stream()
                .mapToInt(e -> e.getKey().getHourlyWage() * e.getValue())
                .sum();
        Map<Month, Integer> monthlyWorkingDays = this.getNumOfWorkingDaysInMonth();
        monthlyWorkingDays.replaceAll((k, v) -> v * totalSalaryPerDay);
    ...
    public void addCommitment(Employee employee, int hoursPerDay) {
        ...
        int hours = this.getCommittedHoursPerDay().getOrDefault(employee, 0);
        this.getCommittedHoursPerDay().put(employee, hours + hoursPerDay);
    ...
    public int calculateManpowerBudget() {
        int dailySalary = committedHoursPerDay.entrySet().stream()
                .mapToInt(e -> e.getKey().getHourlyWage() * e.getValue())
                .sum();
        int workingDays = this.getNumWorkingDays();
    ...
    public boolean equals(Object o) {
        return (this.getCode().equals(((Project) o).getCode()));
    }
